...
Para lograrlo, se utiliza un modelo de Machine Learning (`RandomForestRegressor`) no como una herramienta para predicción, sino como una herramienta de **interpretación** para extraer la **importancia de las variables** (`feature_importance_`).

---

## 2. Stack Tecnológico y Datos

* **Datos:** API del [Sistema de Información Económica (SIE) de Banxico](https://www.banxico.org.mx/SieAPIRest/service/v1/token).
...
### Fase 1: Ingeniería y Exploración de Datos (EDA)

1.  **Extracción:** Se construyó una función (`banxico_utils.py`) para conectarse a la API del SIE y descargar las 3 series de tiempo desde 2008 hasta la fecha.
2.  **Limpieza:** El desafío principal fue la **homologación de frecuencias**. Para crear un DataFrame maestro mensual, se aplicó una lógica distinta a cada serie:
    * **Tipo de Cambio (Diario):** Se remuestreó a mensual usando el promedio (`.resample('M').mean()`).
    * **Tasa Objetivo (Decisión):** Se usó la lógica de "estado vigente", tomando el último valor conocido a fin de mes (`.resample('M').asfreq().ffill()`), evitando así la creación de datos promedio falsos.
3.  **Exploración:** Un `heatmap` de correlación simple reveló un hallazgo clave: una alta correlación positiva entre la Tasa y la Inflación *en el mismo mes*. Esto no demostraba el impacto de la Tasa, sino la **"función de reacción" de Banxico** (Banxico sube la tasa *porque* la inflación sube). Esto justificó la necesidad de usar un modelo de rezagos.

### Fase 2: El Hallazgo del "Cambio de Régimen"

1.  **Ingeniería de Variables:** Se transformó el problema de series de tiempo en uno de **regresión supervisada** usando `.shift()` para crear las variables `X` (rezagos de Tasa, TC, e Inercia de Inflación) para predecir la `Y` (Inflación actual).
2.  **Fracaso de la Predicción:** Al dividir los datos con `train_test_split(shuffle=False)` (para respetar el orden temporal), el modelo falló catastróficamente.
    * **R² Train (2009-2021):** `~0.99` (sobreajuste extremo).
    * **R² Test (2022-2025):** **¡Negativo!** (ej. `-40.75`).
3.  **Diagnóstico:** Se intentó hacer una poda extrema (`max_depth`, `min_samples_leaf`), pero los resultados no cambiaron. Esto demostró un **"cambio de régimen" estructural**. Las reglas económicas que el modelo *memorizó* en el periodo pre-COVID (2009-2021) se rompieron y fueron inútiles para predecir el mundo post-COVID (2022-2025).

### Fase 3: Pivote a la Interpretación
...
1.  **Sin Predictor Dominante:** Se excluyó la variable `Inflacion_lag_1M` para evitar el **"enmascaramiento de variables"** (feature masking) que impedía al modelo ver la importancia de la Tasa y el TC.
2.  **Con Poda Extrema:** Se usaron hiperparámetros restrictivos (`max_depth=3`, `min_samples_leaf=10`) para forzar al modelo a encontrar solo los patrones más simples y robustos.
